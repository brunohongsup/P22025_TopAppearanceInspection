...
BOOL XIni::Open(LPCTSTR szPathName)
{
	...
	// 파일 열기
	hFile = ::CreateFile(szPathName, GENERIC_READ | GENERIC_WRITE, FILE_SHARE_READ, NULL, OPEN_EXISTING, FILE_FLAG_SEQUENTIAL_SCAN, NULL);
	...
	if(nDataLen==0)
	{
		TCHAR szBuf = _T('\n');
		DWORD dwWritten = 0;
		::WriteFile(hFile, &szBuf, sizeof(szBuf), &dwWritten, NULL);
		::SetFilePointer(hFile, 0, NULL, FILE_BEGIN);
		//CloseHandle(hFile);
		//return TRUE;
		nDataLen = (int)dwWritten;
	}
	// 0 바이트이다..
	pData = (BYTE*)calloc(1, nDataLen+sizeof(TCHAR));
