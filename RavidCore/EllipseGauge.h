...
namespace Ravid
{
	namespace Algorithms
	{
		...
		/**
		* 이미지의 타원형 객체를 측정하여 추정된 결과를 제공하는 클래스입니다.
		*/
		...
		public:
			...
			* param[in] dblDiameter1 : 객체 x축 지름
			* param[in] dblDiameter2 : 객체 y축 지름
			...
			* 측정하기 위한 위치, 형태를 설정합니다.
			*
			* param[in] refGeometry : 중심 위치, 반지름, 탐색 시작, 구간, 회전 정보(Pointer)
			* param[in] dblTolerance : 탐색 영역
			*
			* return value : 실패하면 EAlgorithmResult_OK 이외의 값을 가집니다.
			*/
			...
			* 화소값 변화 탐색 방법을 설정함니다.
			*
			* param[in] eTransitionType : enum ETransitionType 화소값 변화 탐색 방법
			*/
			...
			* 탐색 보정 방법을 설정함니다.
			*
			* param[in] eTransitionChoice : enum ETransitionChoice 보정 방법
			*/
			...
			*
			* return value : CRavidEllipse 클래스 형태로 반환합니다.
			*/
			...
			* 입력된 인덱스의 추정 객체를 가져옵니다.
			*
			* param[out] refGeometry : CRavidEllipse 결과 객체 (Reference)
			* param[in_opt] nIndex : 결과 인덱스 (Default : 0)
			*
			* return value : 실패하면 EAlgorithmResult_OK 이외의 값을 가집니다.
			*/
			...
			* 입력된 인덱스의 추정 객체를 가져옵니다.
			*
			* param[out] pGeometry : CRavidEllipse 결과 객체 (Pointer)
			* param[in_opt] nIndex : 결과 인덱스 (Default : 0)
			*
			* return value : 실패하면 EAlgorithmResult_OK 이외의 값을 가집니다.
			*/
