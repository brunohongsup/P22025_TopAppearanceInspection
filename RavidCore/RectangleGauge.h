...
namespace Ravid
{
	namespace Algorithms
	{
		...
		class CRavidImage;
		/**
		* 이미지의 직사각형 객체를 측정하여 추정된 결과를 제공하는 클래스입니다.
		*/
		...
		public:
			...
			* param[in] dblWidth : 객체 가로 길이
			* param[in] dblHeight : 객체 세로 길이
			* param[in] dblTolerance : 탐색 영역
			* param[in] dblAngle : 탐색 회전 각도
			*
			* return value : 실패하면 EAlgorithmResult_OK 이외의 값을 가집니다.
			*/
			EAlgorithmResult SetMeasurementRegion(_In_ CRavidPoint<double>& refGeometry, _In_ double dblWidth, _In_ double dblHeight, _In_ double dblTolerance, _In_ double dblAngle);
			...
			* return value : 실패하면 EAlgorithmResult_OK 이외의 값을 가집니다.
			*/
			EAlgorithmResult SetMeasurementRegion(_In_ CRavidPoint<double>* pGeometry, _In_ double dblWidth, _In_ double dblHeight, _In_ double dblTolerance, _In_ double dblAngle);
			
			/**
			* 실행 전 사용합니다.
			* 측정하기 위한 위치, 형태를 설정합니다.
			*
			* param[in] refGeometry : 객체 형태, 위치 (Reference)
			* param[in] dblTolerance : 탐색 영역
			*
			* return value : 실패하면 EAlgorithmResult_OK 이외의 값을 가집니다.
			*/
			EAlgorithmResult SetMeasurementRegion(_In_ CRavidRect<double>& refGeometry, _In_ double dblTolerance);
			
			/**
			* 실행 전 사용합니다.
			* 측정하기 위한 위치, 형태를 설정합니다.
			*
			* param[in] pGeometry : 객체 형태, 위치 (Pointer)
			* param[in] dblTolerance : 탐색 영역
			*
			* return value : 실패하면 EAlgorithmResult_OK 이외의 값을 가집니다.
			*/
			EAlgorithmResult SetMeasurementRegion(_In_ CRavidRect<double>* pGeometry, _In_ double dblTolerance);
			...
			*
			* return value : CRavidRect 클래스 형태로 반환합니다.
			*/
			...
			* return value : 실패하면 EAlgorithmResult_OK 이외의 값을 가집니다.
			*/
			EAlgorithmResult GetMeasuredObject(_Out_ CRavidRect<double>& refGeometry, _In_opt_ int nIndex = 0);
			...
			* return value : 실패하면 EAlgorithmResult_OK 이외의 값을 가집니다.
			*/
			EAlgorithmResult GetMeasuredObject(_Out_ CRavidRect<double>* pGeometry, _In_opt_ int nIndex = 0);
